...
def main():
    ...
    encoder = Encoder(data_set.in_lang.token_n, embed_size=embed_size, hidden_size=hidden_size, batch_size=batch_size,
                      num_layers=num_layers, drop_prob=drop_prob).to(device)
    decoder = Decoder(vocab_size=data_set.out_lang.token_n, embed_size=embed_size, hidden_size=hidden_size,
                      batch_size=batch_size, num_layers=num_layers, attention_size=attention_size,
    ...
    for epoch in range(num_epochs):
        for batch_idx, (in_seq, out_seq) in enumerate(loader):
            ...
            enc_outputs, enc_hidden = encoder(in_seq, encoder.init_hidden(device=device))
            print(f"enc_outputs shape: {enc_outputs.shape}, enc_hidden shape: {enc_hidden.shape}")
            # 解码器在最初时间步的输入是BOS
            dec_input = decoder.init_input(device=device)
            # initialize hidden state of decoder
            dec_hidden = decoder.init_hidden(enc_hidden)
            # mask [batch_size]
            mask = torch.ones(batch_size, device=device)
            eos = torch.LongTensor([2] * batch_size).to(device)
            pad = torch.zeros(batch_size).to(device)
